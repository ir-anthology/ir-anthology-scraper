...
log_file_path = "copy_pdfs_log.txt"
entry_count = 0
pdf_count = 0
...
for bibfilepath_count, bibfilepath in enumerate(bibfilepaths, 1):
    with open(bibfilepath) as bibfile:
        ...
        for entry in entries:
            ...
            if exists(pdf_dst_path):
                pdf_count += 1
                with open(log_file_path, "a") as logfile:
                    logfile.write("   - " + entry["ID"] + ".pdf" + "\n")
            else:
                if "doi" in entry:
                    pdf_src_path = ir_anthology_root + "sources/wlgc/papers-by-doi/" + entry["doi"] + ".pdf"
                    if exists(pdf_src_path):
                        if not DRY_RUN: copyfile(pdf_src_path, pdf_dst_path)
                        pdf_count += 1
                        with open(log_file_path, "a") as logfile:
                            logfile.write("   - " + entry["ID"] + ".pdf" + "\n")
